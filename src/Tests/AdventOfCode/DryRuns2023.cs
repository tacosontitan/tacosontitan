namespace Sandbox.QuickTests.AdventOfCode;

public class DryRuns2023
{
    [Fact]
    public void DayOne_PartOne()
    {
        // Arrange
        const string input = @"sixrrmlkptmc18zhvninek
jcb82eightwond
...
jmgnfive7ffglffpjlvbtvl935zz";
        ...
        string[] lines = input.Split(splitters, StringSplitOptions.RemoveEmptyEntries);
        foreach (string line in lines)
        {
            char firstDigit = line.FirstOrDefault(c => Char.IsDigit(c));
            char lastDigit = line.LastOrDefault(c => Char.IsDigit(c));
            string rawValue = $"{firstDigit}{lastDigit}";
            int calibrationValue = int.Parse(rawValue);
            calibrationValues.Add(calibrationValue);
        }

        int sum = calibrationValues.Sum();
